...
@Component({
  selector: 'app-promotion-rule-setting-add-customized',
  templateUrl: './promotion-rule-setting-add-customized.component.html',
  styleUrls: ['./promotion-rule-setting-add-customized.component.scss'],
  providers: [ChecklistDatabase],
})
export class PromotionRuleSettingAddCustomizedComponent implements OnInit {
  ...
  createForm(setEmptyString = false) {
    ...
    this.postSettingFG = this.formBuilder.group({
      ecPromotionTitle: new FormControl({ value: setEmptyString ? '' : null, disabled: false }),
      lang: new FormControl({ value: 'TH', disabled: false }),
      deskBanner: new FormControl(
        { value: setEmptyString ? '' : null, disabled: false },
        Validators.required
      ),
      deskGrid: new FormControl({ value: setEmptyString ? '' : null, disabled: false }),
      deskDetail: new FormControl(
        { value: setEmptyString ? '' : null, disabled: false },
        Validators.required
      ),
      mobileBanner: new FormControl({ value: setEmptyString ? '' : null, disabled: false }),
      mobileGrid: new FormControl({ value: setEmptyString ? '' : null, disabled: false }),
      mobileDetail: new FormControl({ value: setEmptyString ? '' : null, disabled: false }),
      mobileAppBanner: new FormControl({ value: setEmptyString ? '' : null, disabled: false }),
      mobileAppDetail: new FormControl({ value: setEmptyString ? '' : null, disabled: false }),
    });

    this.secondFormGroup = this.formBuilder.group({
      validPeriod: [1],
      verificationType: new FormControl({ value: 'Manual', disabled: true }),
      timeGapLimitationEnable: new FormControl(false),
      timeGapLimitationValue: new FormControl({ disabled: true, value: setEmptyString ? '' : null }),
      timeGapLimitationType: ['Min'],
      limitation: [],
      allowApp: ['All'],
      ...
      violationCountSetting: new FormControl({ value: setEmptyString ? '' : null, disabled: true }),
    });
    this.checkTimeGapLimitation();
    ...
    this.thirdFormGroup = this.formBuilder.group({
      ...
      amountLimitType: new FormControl('Limit'),
      amountLimit: [],

      removeGeneral: new FormControl('Yes'),
      depositAmt: [],
  ...
  checkTimeGapLimitation() {
    this.secondFormGroup.controls.timeGapLimitationEnable.valueChanges.subscribe(
      (data) => {
        if (data) {
          this.secondFormGroup.controls.timeGapLimitationValue.enable();
          this.secondFormGroup.controls.timeGapLimitationValue.setValidators(
            Validators.required
          );
        } else {
          this.secondFormGroup.controls.timeGapLimitationValue.disable();
          this.secondFormGroup.controls.timeGapLimitationValue.clearAsyncValidators();
          this.secondFormGroup.controls.timeGapLimitationValue.clearAsyncValidators();
        }
        this.secondFormGroup.controls.timeGapLimitationValue.updateValueAndValidity();
      }
    );
  }
  ...
  openDialogShowSetting(data: DialogReqData = null) {
    const dialogRef = this.dialog.open(PromotionRuleSettingReviewDialogComponent, {
      data: data,
      width: '95%',
      height: '95%',
      maxHeight: '98%'
  ...
  editOne(body) {
    DialogSweetAlertService.opentModalSweetAlertConfirm('', MessageService.DIALOGMSGCONFIRM.EDIT, () => {
      this.httpCli.doPut(`promotion/edit-promotion/${this.promoCode}`, body).subscribe((res) => {
        if (res.status === MessageService.MSG.SUCCESS) {
          this.location.back();
          this.disabledBtn = false;
          DialogSweetAlertService.opentModalSweetAlertSuccess('', res.message);
  ...
  getPromoDetail(promoCode) {
    this.httpCli.doGet(`promotion/get-by-promo-code?promoCode=${promoCode}`).subscribe((res) => {
      if (res.status === MessageService.MSG.SUCCESS) {
  ...
  setForm(data: PromotionDetailModel) {
    ...
    this.thirdFormGroup.patchValue({
      ...
      maxApprovalLimitType: data.ruleSetting.maxApprovalLimit == 9999999999 ? 'Unlimited' : 'Limit',
      maxBonusLimitType: data.ruleSetting.maxBonusLimit == 9999999999 ? 'Unlimited' : 'Limit',
    ...
    data.issueSetting.forEach(element => {
      let node = this.treeControl.dataNodes.find(ele => ele.item.gpCode == element.providerCode && ele.item.gpNameEn == element.providerName)
      if (node)
        this.checklistSelection.select(node)
    });
  }
